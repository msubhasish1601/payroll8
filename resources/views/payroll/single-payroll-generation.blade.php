                    ...
                    <div class="card-body card-block">
                        <form action="{{ url('payroll/add-payroll-generation') }}" method="post"
                            ...
                            <div class="row form-group demo">
                                ...
                                <div class="col-sm-2">
                                    ...
                                    <div class="input-group mb-3">
                                        <input class="demo-1 form-control" id="month_yr" type="date" value="<?php echo date('Y-m-d'); ?>" name="month_yr" />
                            ...
                            <div class="row form-group">
                                ...
                                <div class="col-md-3">
                                    <label>PF</label>
                                    <input name="emp_pf" id="emp_pf" type="text" class="form-control" readonly="1"
                                ...
                                <div class="col-md-3">
                                    <label>TDS</label>
                                    <input type="text" name="emp_tds_ded" id="emp_tds_ded" class="form-control"
                                        readonly="1" onblur="OnBlurCalculateSubtraction();">
                                </div>
                                <!-- <div class="col-md-3">
                                    <label>Income Tax</label>
...
function getEmpCode() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            var no_of_working_days = current_month_days;
            var no_of_present = current_month_days;
            var no_of_days_absent = 0;
            var no_of_days_salary = 0;
            var no_of_tour_leave = 0;
            var no_of_leave = 0;
            ...
            for (var j = 0; j < obj[3].length; j++) {
                ...
                } else if (obj[3][j].rate_id == '31') {
                    if (obj[0].tds == '1') {
                        if (obj[3][j].inpercentage != '0') {
                            tds = Math.round(basic * obj[3][j].inpercentage / 100);
                            $("#emp_tds_ded").val(tds);
                        } else {
                            if ((basic <= obj[3][j].max_basic) && (basic >= obj[3][j].min_basic)) {
                                $("#emp_tds_ded").val(obj[3][j].inrupees);
                            }
                        }
                        $("#emp_tds_ded").prop("readonly", true);

                    } else if (obj[0].tds != null && obj[0].tds != '') {
                        $("#emp_tds_ded").val(obj[0].tds);
                        $("#emp_tds_ded").prop("readonly", false);
                    } else {
                        tds = 0;
                        $("#emp_tds_ded").val(tds);
                        $("#emp_tds_ded").prop("readonly", true);
...
function OnBlurCalculateSubtraction() {
    ...
    var total_deduction = (parseFloat($('#emp_prof_tax').val()) + parseFloat($('#emp_co_op').val()) +
                parseFloat($('#emp_pf').val()) + parseFloat($('#emp_pf_employer').val()) + parseFloat($('#emp_pf_int').val()) + parseFloat($('#emp_apf').val()) + parseFloat($('#emp_i_tax').val()) + parseFloat($('#emp_pf_loan').val()) + parseFloat($('#emp_insu_prem').val()) + parseFloat($('#emp_esi').val()) + parseFloat($('#emp_adv').val()) + parseFloat($('#emp_hrd').val()) + parseFloat($('#emp_furniture').val()) + parseFloat($('#emp_misc_ded').val()) + parseFloat($('#emp_tds_ded').val())+ parseFloat(other_deduction));
