...
<br>
        ...
        if ($leave_allocation_rs) {$f = 1;
            ...
            foreach ($leave_allocation_rs as $leave_allocation) {
                ...
                if ($leave_allocation->file != '') {
                    $file = '
			   <a href="https://app.theglobalrecruitment.co.uk/' . $leave_allocation->file . '" data-toggle="tooltip" data-placement="bottom" title="Download" download>

              <img style="width: 14px;" src="https://app.theglobalrecruitment.co.uk/assets/img/download.png"></a>';
                } else {
                    $file = '';
                }
    ...
    public function viewattendancemonthwise($emp_id, $month_yr)
    {
        ...
        try {
            ...
            $daliyEmployee = DB::table('attandence')->select(DB::raw('DISTINCT date'))->distinct('date')
                ->where('employee_code', '=', $Employeeus->employee_id)->where('month', '=', base64_decode($month_yr))->orderBy('id', 'asc')->get();
            if (count($daliyEmployee) != 0) {

                foreach ($daliyEmployee as $value) {

                    $attndetails = array('date' => $value->date, 'month' => base64_decode($month_yr));
                    $daliyEmployeedate = DB::table('attandence')->where('employee_code', '=', $Employeeus->employee_id)->where('month', '=', base64_decode($month_yr))->where('date', '=', $value->date)->orderBy('id', 'asc')->get();
    ...
    {try {
        $date = date('Y-m-d');
        $daliyEmployee = DB::table('attandence')->select(DB::raw('DISTINCT employee_code'))->distinct('employee_code')
        ...
        if (count($daliyEmployee) != 0) {
            foreach ($daliyEmployee as $value) {
                ...
                $Roledata = DB::table('employees')->join('users', 'employees.emp_code', '=', 'users.employee_id')
                    ...
                    ->select('employees.emp_code', 'employees.emp_fname', 'employees.emp_mname', 'employees.emp_lname', 'employees.emp_ps_email')
                    ->first();
    ...
    public function allleavreqe($employee_id)
    {
        ...
        try {
            ...
            $Employee = DB::table('employees')->where('emp_code', '=', $employee_id)->first();
            ...
            $data['LeaveAllocation'] = DB::table('leave_allocations')
                ->join('leave_types', 'leave_allocations.leave_type_id', '=', 'leave_types.id')
                ->where('leave_allocations.employee_code', '=', $employee_id)
            ...
            //->whereDate('leave_allocation.created_at','>=',$first_day_this_year)
                ->select('leave_allocations.*', 'leave_types.leave_type_name', 'leave_types.alies', DB::raw("(select sum(leave_applies.no_of_leave) from `leave_applies` where `leave_applies`.`employee_id` = '" . $employee_id . "' and `leave_applies`.`status` = 'APPROVED' and `leave_applies`.`from_date` between '2023-01-01' and '2023-12-31' and `leave_applies`.`to_date` between '" . $first_day_this_year . "' and '" . $last_day_this_year . "' and `leave_applies`.`leave_type`=leave_allocations.leave_type_id) as leave_taken"))
                ->orderBy('leave_types.leave_type_name', 'ASC')
                ->get();
